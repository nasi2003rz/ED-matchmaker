...
## End-to-end testing (CLAUDE.md Section 12, Step 21)

Until this step the project had zero real tests — only Nest's default boilerplate (`GET /` → "Hello World!"). Section 13's Definition of Done lists "Tests" as required for every feature, and Section 5.1 is explicit about what "done" means for Phase 1: *"If this workflow works reliably end-to-end, Phase 1 has achieved its objective."* This step makes that an automated, repeatable check instead of a one-off manual pass.

**What was added** (`backend/test/`, run with `npm run test:e2e`):

- `critical-workflow.e2e-spec.ts` — walks Section 5.1's whole Phase-1 path in one continuous, stateful story against the real app + real dev database: instructor registers → completes their profile → creates a class with a weekly schedule (sessions auto-generate) → student registers → discovers the class via the public invitation preview → joins through the invitation link → shows up on the roster/the student's own calendar/"my classes" → instructor marks attendance → creates homework → student submits it → instructor reviews and scores it → instructor records a grade → instructor and student exchange messages (unread count, read receipt) → instructor creates a tuition invoice and records a partial payment → both the student's and the parent's dashboards/payment views reflect all of it → parent registers, links the student as a child, sees them on the family calendar. 30 assertions, one continuous journey.
- `authorization.e2e-spec.ts` — CLAUDE.md Section 9 is non-negotiable, so this suite specifically tries the cross-tenant access every service is supposed to refuse: a second, unrelated instructor reading/editing/grading/invoicing/marking-attendance-in the first instructor's class; a student who never enrolled submitting to its homework; an outsider (student or instructor) reading a conversation they're not a party to; a role-gated route hit with the wrong role; every protected route hit with no token or a garbage token. 18 assertions, all expecting a refusal.
- `test/utils/test-app.ts` — a shared helper that boots the Nest app wired exactly like `src/main.ts` (global `api` prefix, the whitelist/transform `ValidationPipe`, cookie parsing), so requests behave identically to hitting the real running server; plus an explicit, exact-email `cleanupUsers` helper (never a broad delete) that every spec's `afterAll` uses, relying on the schema's cascade deletes to remove everything a test user owns.
- Fixed `app.e2e-spec.ts`'s pre-existing broken `import { App } from 'supertest/types'` (that subpath doesn't exist in the installed `supertest` version — the boilerplate test only ever passed because nothing exercised the type-checked import path until this step touched the file).

**Scope note, stated plainly:** this project has no disposable per-run test database yet — these e2e tests run against the same local dev Postgres `docker compose up` already requires, using a random per-run email suffix to avoid unique-constraint collisions and deleting everything they create afterward (verified: 0 leftover rows after a run, and running the full suite three times back-to-back all pass cleanly). A CI pipeline with an isolated test database is future infrastructure, not something Phase 1 needs yet. Frontend automated testing (Playwright/Cypress or similar) was **not** added in this step — there is no frontend test runner in this repo at all yet, and standing one up is a real infrastructure decision on its own; the frontend's critical flows are instead covered by the manual, real-browser walkthroughs already done across Steps 18–20 (every role, every changed page, desktop and mobile) plus a final live smoke pass this step: registering an instructor and creating a class through the actual UI forms (not the API) end-to-end against a running backend, confirming the two layers are still wired together correctly after every backend change made in Steps 18–21.

**Result:** 48/48 backend e2e tests pass (30 critical-workflow + 18 authorization), plus the pre-existing 1 unit test. `npm run test:e2e` runs them; `npm run test` runs the unit suite.
